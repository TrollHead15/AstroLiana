...
export const GuideForm = ({ isOpen, onClose }: GuideFormProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      trackEvent("modal_opened", { leadType: "guide" });
    } else {
      reset(defaultValues);
    }
  ...
  const submitHandler = async (values: LeadPayload) => {
    ...
    try {
      ...
      trackEvent("form_submitted", { leadType: "guide" });
      trackEvent("email_sent", { leadType: "guide" });
      toast.success("Гайд отправлен на указанный email");
      onClose();
      reset(defaultValues);
    } catch (error) {
      console.error("[GuideForm] submit", error);
  ...
  return (
    <LeadMagnetModal isOpen={isOpen} onClose={onClose} title="Получить гайд">
      <form onSubmit={handleSubmit(submitHandler)} className="space-y-5">
        ...
        <div className="pt-2">
          <Button type="submit" variant="accent" className="w-full" disabled={isSubmitting}>
            {isSubmitting ? "Отправляем..." : "Отправить гайд"}
          </Button>
        </div>
      </form>
    </LeadMagnetModal>
